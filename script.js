...
// LUT must hold whichever spiral type uses the most area for the same N:
//   - Ulam (square rings):    max |coord| ≈ √N / 2
//   - Euclidean (polar):      max |coord| ≈ √(N/π)
//   - Diamond (Manhattan):    max |coord| ≈ √(N/2)
// Diamond is the biggest, so we size for it always — switching spiral type
// at runtime doesn't need to reallocate the LUT.
...
// --- Three spiral generators, each with its own state.
//
...
//
// Euclidean (polar) spiral: cells sorted primarily by x²+y² (Euclidean
// distance squared), and cells at the same distance ordered counter-
// clockwise by atan2(y, x). Cannot be generated by a simple walker — we
// enumerate the lattice points in a disk and sort. Whole-spiral
// regenerate on each grow.
let spiralType = 'ulam';
...
// Tracks the MAX_SPIRAL value the Euclidean spiral was last generated for —
// so we only regenerate when capacity grows, not on every advanceSpiralTo
// call. Also doubles as the source of truth for the Euclidean ring sizes.
let euclideanGeneratedFor = 0;
// Ring r (= floor(√(x²+y²))) → number of cells in the generated spiral at
// that ring. Used by the stats panel for "% unfilled per ring" since
// ringSize(r) isn't a closed-form formula in the Euclidean case.
...
function advanceSpiralTo(target) {
  if (spiralType === 'euclidean') {
    if (euclideanGeneratedFor !== MAX_SPIRAL) regenerateEuclideanSpiral();
  } else if (spiralType === 'ulam') {
    advanceUlamTo(target);
  } else {
    advanceDiamondTo(target);
  }
}

function activeWalkerCount() {
  if (spiralType === 'ulam') return ulamWalker.count;
  if (spiralType === 'diamond') return diamondWalker.count;
  return euclideanGeneratedFor; // Euclidean: count = current MAX_SPIRAL
}

// Reset all generators and the LUT, then regenerate from scratch.
...
function resetSpiral() {
  ...
  euclideanGeneratedFor = 0;
  euclideanRingSize = null;
  spiralIndexLUT.fill(0);
  spiralX[0] = 0; spiralY[0] = 0;
  spiralIndexLUT[MAX_X * LUT_SIDE + MAX_X] = 1;
...
//   - Ulam:      Chebyshev distance, ring r has 8r cells (1 for r=0)
//   - Diamond:   Manhattan distance, ring r has 4r cells (1 for r=0)
//   - Euclidean: floor(√(x²+y²)) — ring size and totals come from
//                pre-computed euclideanRingSize[], since neither is
//                a closed-form expression.
function ringOf(x, y) {
  if (spiralType === 'diamond') return Math.abs(x) + Math.abs(y);
  if (spiralType === 'euclidean') return Math.floor(Math.sqrt(x * x + y * y));
  return Math.max(Math.abs(x), Math.abs(y));
}
function ringSize(r) {
  if (spiralType === 'euclidean') {
    return (euclideanRingSize && euclideanRingSize[r]) || 0;
  }
  if (r === 0) return 1;
...
function totalCellsThroughRing(r) {
  if (spiralType === 'euclidean') {
    let total = 0;
    for (let i = 0; i <= r; i++) total += ringSize(i);
    return total || 1;
  }
  if (r <= 0) return 1;
